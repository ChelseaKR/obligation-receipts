...
`unreadable` (over the cap, not a regular file, not strict JSON), with counts per
obligation and overall; for an attestation it also reports whether the document
parses and whether its binding fields match the manifest. It never reads the
attestation's own `status` value and never reports an assertion outcome — those
are the verdict, and the point of this command is that collection state and
verdict stay separate, because missing evidence is not failure. Exit 0 whenever
the report was computed; 2 only when the plan does not regenerate from the
manifest, or an input cannot be read. Redacted by default like the plan, with
`--include-local-details` and `--markdown`.
...
| CI/CD | Applies — hosted CI on `push` and `pull_request`, carried by `ci.yml` since the first commit; the `protect-main` ruleset, active since 2026-08-07, requires a pull request and six of the seven jobs `ci.yml` defines (`verify`, `package`, `dependency-scan`, `secret-scan`, `sast`, `zizmor`), having required only `verify` until the change recorded in the CHANGELOG; `dogfood-action` — which runs this repository's own composite action against its own example on every pull request — is **not** yet named by the ruleset, so like `codeql` it reports without being able to block, and the ruleset must be extended to name all seven checks before that gap closes; gap tracked in #16 for the CodeQL `language: actions` element, now unwaived — WVR-008 was retired when this repository became public, because code scanning is available here and the analysis is simply unconfigured; `codeql.yml` now runs that analysis on every pull request, but it is **not** a required check until the `protect-main` ruleset names it, so it reports without being able to block |
| Release & Versioning | Applies — `release.yml` is dispatched against an existing signed annotated tag, which a standards-owned authorization job verifies against `.github/allowed_signers` and proves reachable from `main`; the gates re-run at that exact commit, and the build emits an SBOM, a build-provenance attestation, and a keyless signature over `dist/SHA256SUMS`; publication is confined to two checkout-free jobs that republish the attested bytes without rebuilding — a GitHub release and a PyPI upload over Trusted Publishing with no stored credential, which `tests/test_supply_chain.py` asserts. WVR-009, which waived this control while the workflow deliberately held no publication authority, was retired on its own stated trigger when that authority was added. **Nothing has been published yet**: no version is on PyPI, and the upload job cannot succeed until a Trusted Publisher is registered there by hand |
