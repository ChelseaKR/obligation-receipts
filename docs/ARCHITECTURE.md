...
- `progress.py` reports collection progress against an approved plan and
  ...
  attestation whether it parses and its binding fields match the manifest. It
  never reads the attestation's own `status` value and never reports an
  assertion outcome: those are the verdict, and this is not it.
