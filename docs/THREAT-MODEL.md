...
| Evidence is regenerated between collection and evaluation | an optional evidence lock digests every declared artifact at collection time; `evaluate --lock` refuses, writing no receipt, when an artifact's digest changed, when one frozen as absent is now present, or when one frozen as present is gone | the lock is unsigned and makes no claim about who took it or when — a party who can rewrite the evidence can rewrite the lock beside it |
| A failed read is frozen as an observation of absence | only a genuine not-found is recorded as `absent`; any other read failure (over the artifact cap, not a regular file, escaping the root) refuses to write a lock at all, naming the evidence id | an artifact that becomes unreadable between the two moments refuses the evaluation rather than reporting why the read failed |
